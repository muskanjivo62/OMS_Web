...
export default function Party_Product_Assignment() {
  ...
  useEffect(() => {
    if (selectedParty) {
      fetchPartyProducts(selectedParty);
    } else {
      setAssignedProducts([]);
    }
  }, [selectedParty]);
  ...
  const fetchPartyProducts = async (card_code: string) => {
    try {
      ...
      // Safely check for nested data structure commonly returned by the backend
      setAssignedProducts(res.data?.data?.products || res.data?.products || []);
    } catch (error) {
  ...
  const handleRemoveProduct = async (product: PartyProduct) => {
    ...
    try {
     const res = await userService.removePartyProduct(selectedParty, product.item_code, product.category);
...
const handleBulkAssign = async () => {
  if (selectedNewProducts.length === 0) return;
  ...
  try {
    ...
    console.log("FINAL PAYLOAD:", {
      card_code: selectedParty,
      products: payload
    });

    await api.post('/auth/party-product/bulk-add/', {
      card_code: selectedParty,
      products: payload
    });
  ...
  const handleEditRate = async (product: PartyProduct) => {
    ...
    try {
       const res = await userService.editRate(selectedParty, product.item_code, product.category, parsedRate);
      console.log("Rate updated successfully", res);
      ...
      alert("Failed to update rate. Please check the API endpoint.");
    }
  };
  ...
  const selectedPartyDetails = parties.find((p) => p.card_code === selectedParty);
      ...
      <div className="pa-header app-page-head">
        <div>
          ...
          <p className="pa-subtitle app-page-subtitle">
            Assign products and specific rates to parties for order processing.
          </p>
        </div>
      </div>
                ...
                filteredParties.map((party) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      setPartySearch(`${party.card_name} (${party.card_code})`);
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px", flexWrap: "wrap", gap: "16px" }}>
            <div>
              ...
              <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                <span style={{ fontSize: "0.875rem", color: "#475569", fontWeight: 600, background: "#f1f5f9", padding: "4px 8px", borderRadius: "4px" }}>
                  {selectedPartyDetails.card_code}
                </span>
                <span style={{ fontSize: "0.875rem", color: "#64748b" }}>
                  {selectedPartyDetails.state || "Unknown State"} • {selectedPartyDetails.main_group || "Unknown Group"}
                </span>
              </div>
            </div>
            <button
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "16px", marginBottom: "24px" }}>
            <div style={{ background: "#f8fafc", padding: "16px", borderRadius: "8px", border: "1px solid #e2e8f0", textAlign: "center" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#0f172a" }}>{totalProducts}</div>
              <div style={{ fontSize: "0.75rem", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "4px" }}>Total Assigned</div>
            </div>
            <div style={{ background: "#fffbeb", padding: "16px", borderRadius: "8px", border: "1px solid #fde68a", textAlign: "center" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#d97706" }}>{oilCount}</div>
              <div style={{ fontSize: "0.75rem", color: "#b45309", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "4px" }}>Oil</div>
            </div>
            <div style={{ background: "#eff6ff", padding: "16px", borderRadius: "8px", border: "1px solid #bfdbfe", textAlign: "center" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#2563eb" }}>{beverageCount}</div>
              <div style={{ fontSize: "0.75rem", color: "#1d4ed8", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "4px" }}>Beverages</div>
            </div>
            <div style={{ background: "#f1f5f9", padding: "16px", borderRadius: "8px", border: "1px solid #cbd5e1", textAlign: "center" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#334155" }}>{martCount}</div>
              <div style={{ fontSize: "0.75rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "4px" }}>Mart</div>
            </div>
          </div>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <span style={{
                          fontSize: "0.7rem", fontWeight: 700, padding: "2px 6px", borderRadius: "4px", color: "#fff",
                          background: product.category === "OIL" ? "#f59e0b" : product.category === "BEVERAGES" ? "#3b82f6" : "#1e3a5f"
                        }}>
                          {product.category}
                        </span>
                        <button
                          style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '1.25rem', cursor: 'pointer', lineHeight: 1, padding: 0 }}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '1.25rem', color: '#0f172a' }}>Add Products to {selectedPartyDetails?.card_name}</h2>
              <button onClick={() => { setShowAddModal(false); setSelectedNewProducts([]); setNewProductRates({}); setModalSearch(""); }} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer', color: '#64748b' }}>×</button>
            </div>
              ...
              {filteredAvailable.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '12px' }}>
                  {filteredAvailable.map(product => {
                    const isSelected = selectedNewProducts.some(p => p.item_code === product.item_code && p.category === product.category);
                    return (
                      <div key={`${product.item_code}-${product.category}`} style={{
                        display: 'flex', alignItems: 'flex-start', padding: '12px', border: '1px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer',
                        background: isSelected ? '#eff6ff' : '#fff', borderColor: isSelected ? '#bfdbfe' : '#e2e8f0', margin: 0
                      }} onClick={() => {
                        if (isSelected) {
                          setSelectedNewProducts(selectedNewProducts.filter(p => !(p.item_code === product.item_code && p.category === product.category)));
                        } else {
                        ...
                        <input
                          ...
                          checked={isSelected}
                          readOnly
                          style={{ marginTop: '4px', marginRight: '12px', width: '16px', height: '16px', accentColor: '#2563eb', cursor: 'pointer', pointerEvents: 'none' }}
                        />
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: '#0f172a' }}>{product.item_name}</div>
                          <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '4px' }}>{product.item_code} • {product.category}</div>
                          {isSelected && (
                            <div style={{ marginTop: '12px' }} onClick={(e) => e.stopPropagation()}>
                              <label style={{ fontSize: '0.75rem', color: '#475569', display: 'block', marginBottom: '4px', fontWeight: 500 }}>Basic Rate (₹)</label>
                              <input
                                type="number" min="0" step="0.01" placeholder="0.00"
                                value={newProductRates[`${product.item_code}-${product.category}`] || ''}
                                onChange={(e) => setNewProductRates(prev => ({ ...prev, [`${product.item_code}-${product.category}`]: e.target.value }))}
                                style={{ width: '100%', padding: '8px 10px', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '0.85rem', outline: 'none', boxSizing: 'border-box' }}
                              />
                            </div>
                          )}
                        </div>
                ...
                <div style={{ padding: '40px 0', textAlign: 'center', color: '#64748b' }}>No products match your search</div>
              )}
            </div>

            <div style={{ padding: '20px 24px', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button onClick={() => { setShowAddModal(false); setSelectedNewProducts([]); setNewProductRates({}); setModalSearch(""); }} style={{ padding: '10px 20px', border: '1px solid #cbd5e1', background: '#fff', borderRadius: '8px', cursor: 'pointer', fontWeight: 500, color: '#475569' }}>
                Cancel
              </button>
              <button onClick={handleBulkAssign} disabled={isSaving || selectedNewProducts.length === 0} style={{ padding: '10px 20px', border: 'none', background: '#2563eb', color: '#fff', borderRadius: '8px', cursor: 'pointer', fontWeight: 500, opacity: (isSaving || selectedNewProducts.length === 0) ? 0.7 : 1 }}>
                {isSaving ? 'Saving...' : `Add ${selectedNewProducts.length} Products`}
              </button>
            </div>
          </div>
        </div>
